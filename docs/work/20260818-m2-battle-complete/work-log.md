...
- 진행 중인 작업: 없음
- 마지막 완료 작업: [TASK-34 자체 리뷰·전체 검증·통합](../../plan.md#task-34-자체-리뷰전체-검증통합) (2026-08-21)
- 차단 요인: **VER-09** — 실행 환경에 Chromium 계열 브라우저가 없다. 브라우저를 갖춘 PC에서 실행하면 해소된다.
...
### 2026-08-19 — TASK-30 일기토

- 수행 내용: [TASK-30](../../plan.md#task-30-일기토)을 TDD 사이클로 구현했다. 선행 테스트 `tests/battle/duel.test.ts`(신규 18건)를 먼저 작성해 `duel.ts` 미존재로 실패(Red)를 확인한 뒤 구현했다. 고정 승패와 `judge` 판정, 승패 효과, `onAWin`/`onBWin` 하위 액션이 이벤트 인터프리터에 붙었다.
- 변경 파일: 신규 `src/core/battle/duel.ts`·`tests/battle/duel.test.ts`, `schemas.ts`(`DuelActionSchema`·`CombatConfig.duel`), `eventRunner.ts`(`duel` 액션), `events.ts`(`duelStarted`·`duelResolved`), `data/config/combat-config.json`, `tests/data/fixtures.ts`
- 발견 사항:
  - **계획의 위험이 실제로 성립했고 그대로 잡혔다.** "일기토 결과로 필수 장수가 사라지는 경우를 `outcome`이 그대로 잡는지"가 [TASK-30의 위험](../../plan.md#task-30-일기토)이었다. 패자를 전장에서 지우기만 하면 `outcome`의 `officerLost` 판정이 생존자 목록으로 돌아가므로 별도 배선이 필요 없었고, 패배 조건 무장이 일기토에서 지는 시나리오를 테스트로 못박아 확인했다.
- 결정과 이유:
  - **액션 계약을 재귀로 넓히지 않았다.** `onAWin`/`onBWin`이 다시 `duel`을 품으면 스키마가 재귀가 되어 `z.lazy`가 필요하다. 일기토 안의 일기토는 쓸 일이 없으므로 액션 유니온을 "중첩 없는 11종"과 "duel"로 나누고, 하위 액션은 앞의 11종만 받게 했다(YAGNI).
  - **하위 액션 실행을 인터프리터에 남겼다.** `duel.ts`가 직접 실행하면 [eventRunner](../../../src/core/campaign/eventRunner.ts)와 서로를 부르게 된다. `resolveDuel`은 승패와 이벤트만 돌려주고, 어느 쪽 액션을 이어 실행할지는 인터프리터가 정한다.
  - **고정 승패는 난수를 쓰지 않는다.** [상세 스펙 §7.2](../../../yeonggeoljeon-remake-spec-detail.md)가 `judge`만 "RNG 소비 1회"로 규정했다. 스크립트가 정한 승패에 난수를 소비하면 같은 스크립트가 뒤따르는 판정을 다르게 만들어 결정론(NFR-01)이 흔들린다 — 소비 횟수를 테스트로 고정했다.
  - **괴멸과 퇴각을 이벤트로 구분했다.** 상태 변화는 둘 다 "전장에서 빠짐"으로 같지만, 괴멸은 `defeated`(잃은 것)이고 퇴각은 `unitRemoved`(떠난 것)다. 캠페인이 장수 영구 이탈을 다루는 M3에서 이 구분이 의미를 갖는다.
  - **`winnerMorale`은 미확정 값이다.** §7.2가 "아군 전체 사기 +N"이라고만 적어 초기값 10을 넣고 config에 두었다 — TASK-33의 원작 대조 대상이다.
- 실행한 검증:

  | 검증 | 명령 | 결과 | 증거 |
  |---|---|---|---|
  | 선행 테스트 실패 확인 (Red) | `npx vitest run tests/battle/duel.test.ts` | 의도한 실패 | `duel.ts` 미존재 |
  | 난수 소비 (NFR-01) | `npm test` | 성공 | 고정 승패는 `rng.save()`가 그대로고, `judge`는 한 번 소비한 상태와 일치한다 |
  | 승패 효과 | `npm test` | 성공 | 승자 경험치 100으로 즉시 레벨업 / 승자 진영 전체 사기 상승 / 패자 진영은 그대로 / 괴멸·퇴각 구분 |
  | 위험 확인 | `npm test` | 성공 | 패배 조건 무장이 일기토에서 지면 `outcome`이 `defeat`을 낸다 |
  | 전체 테스트 | `npm test` | 성공 | 17파일 443건 통과 (TASK-29 시점 425건 + 신규 18건) |
  | 타입 검사·데이터 검증·빌드 | `npx tsc --noEmit` / `npm run validate` / `npm run build` | 성공 | 출력 없음 / `데이터 검증 통과` / `✓ built in 1.04s` |

- 결과: 일기토 규칙이 코어에 들어가고 이벤트 액션으로 발동한다. 컷인 연출은 [TASK-32](../../plan.md#task-32-전투-ui-확장)가 `duelStarted`·`duelResolved` 이벤트를 소비해 만든다.

### 2026-08-19 — TASK-31 2단계 적 AI

- 수행 내용: [TASK-31](../../plan.md#task-31-2단계-적-ai)을 TDD 사이클로 구현했다. 계획대로 **성능 테스트를 먼저** 작성하고([RISK-M2-04](../../plan.md#위험)) 프로필 5종·가중치 스코어링·책략 사용 판단·`setAiProfile`을 구현했다. **VER-11 충족.**
- 변경 파일: `src/core/battle/ai.ts`(재작성), `tests/battle/ai-performance.test.ts`(신규), `schemas.ts`(`CombatConfig.ai`·`StageUnit.ai`·`aiParams`·`setAiProfile`), `state.ts`(`Unit.ai`·`aiParams`), `tactics.ts`(`expectedTacticValue`·`tacticTargetRejection`의 `from`), `eventRunner.ts`·`events.ts`, `data/config/combat-config.json`, 테스트 5파일
- 발견 사항:
  - **성능 테스트는 Red가 아니라 가드레일이 됐다.** M1의 단순 AI에서는 당연히 통과하므로 처음부터 초록이었다. 계획의 의도는 "구현 중 초과를 즉시 보게 한다"였고 그 역할은 그대로 한다 — 전탐색으로 바꾼 뒤에도 적 10부대·20×20 맵에서 100ms 안에 끝나는 것을 확인했다.
  - **M1 AI 테스트 3건이 "최근접" 규칙을 단언하고 있었다.** 2단계 AI는 거리뿐 아니라 위협도·지형·위험을 함께 저울질하므로 더 먼 상대를 고를 수 있다. 회귀가 아니라 [FR-11](../../requirements.md#기능-요구사항)이 요구한 규칙 교체이므로, 각 테스트의 의도를 "공격한다"·"같은 배치는 같은 수를 낸다"로 좁혀 남겼다. 어느 대상을 고르는지는 이제 가중치가 정한다.
  - **갇힌 부대 테스트가 책략 때문에 깨졌다.** 산에 갇혀도 사거리 안이면 책략을 쓸 수 있어 대기하지 않는다 — 올바른 새 동작이라 배우는 책략이 없는 기병으로 바꿔 "정말 할 일이 없을 때 대기한다"를 유지했다.
- 결정과 이유:
  - **가중치 6종을 전부 config에 두었다.** [상세 스펙 §5.2](../../../yeonggeoljeon-remake-spec-detail.md)가 `W1~W6`을 `combat-config.json`의 AI 섹션으로 지정했고, 난이도 프리셋도 "가중치+명중 보정 세트"이므로 난이도 교체가 데이터 수정으로 끝난다(NFR-06).
  - **`받을 반격 위험`을 인접 상대 수로 대신 잰다.** 반격이 기본 규칙이 아니므로(§1.1) 스펙 자신이 "인접 적 수·다음 턴 피격 예상으로 대체"하라고 적었다. 그대로 따랐다.
  - **후보 폭발을 미리 잘랐다.** 책략 후보는 `mp`가 모자란 것을 먼저 걸러내고, 회복 책략은 아군에게만·공격 책략은 상대에게만 건다. 기대값이 0 이하인 후보(사기저하·혼란처럼 수치로 재지 않는 효과)도 버린다 — 남는 후보가 줄어 100ms 예산 안에 든다.
  - **`tacticTargetRejection`에 `from` 좌표를 더했다.** AI는 "그 칸에 섰다고 가정한" 판정이 필요한데, 게이트 규칙을 AI가 따로 구현하면 두 곳이 갈라진다. `inAttackRange`가 이미 같은 방식으로 `from`을 받고 있어 그 선례를 따랐다.
  - **동점은 후보 키로 깨뜨린다.** 점수가 같으면 `유형:대상:좌표` 문자열이 작은 쪽을 고른다. 같은 배치가 언제나 같은 수를 내야 하기 때문이다(NFR-01) — M1이 "무장 ID 순"으로 지키던 성질의 확장이다.
  - **`defensive`의 각성을 `planUnitTurn` 안에 두었다.** 계획이 상태를 바꾸는 것이 이상해 보이지만, 각성은 "무엇을 할지 정하는 과정에서 일어나는 영구 전환"이라 그 자리가 맞다(§5.1). 한 번 깨어나면 되돌아가지 않는 것을 테스트로 고정했다.
  - **`support`는 다친 아군이 있으면 회복 후보만 남긴다.** 그러지 않으면 공격 점수가 회복을 눌러 프로필이 무의미해진다. 회복할 수 없을 때만 일반 스코어링으로 넘어간다.
- 실행한 검증:

  | 검증 | 명령 | 결과 | 증거 |
  |---|---|---|---|
  | **VER-11**(AC-12, 성능) | `npx vitest run tests/battle/ai-performance.test.ts` | 성공 | 적 10부대·20×20 맵의 페이즈 전체와 책략 후보가 많은 배치 모두 100ms 미만 |
  | 프로필 5종 | `npm test` | 성공 | `defensive` 경계 거리 밖 대기 → 안으로 들어오면 영구 전환 / `guard` 자리 고수·사거리 내 공격 / `flee` 탈출점 이동 / `support` 회복 우선·위험 시 후퇴 / `aggressive` 스코어링 |
  | 책략 사용 판단 | `npm test` | 성공 | 기대 피해가 앞서고 책략치가 넉넉하면 책략을, 모자라면 통상 공격을 고른다 |
  | 결정론 (NFR-01) | `npm test` | 성공 | 같은 배치가 같은 커맨드 열을 낸다 |
  | 페이즈 종료 보장 | `npm test` | 성공 | 어떤 배치에서도 적 부대가 모두 `acted`로 끝난다(M1이 보장한 성질) |
  | 전체 테스트 | `npm test` | 성공 | 18파일 456건 통과 (TASK-30 시점 443건 + 신규 13건) |
  | 타입 검사·데이터 검증·빌드 | `npx tsc --noEmit` / `npm run validate` / `npm run build` | 성공 | 출력 없음 / `데이터 검증 통과` / `✓ built in 1.03s` |

- 결과: 적 AI가 프로필과 가중치로 움직이고 이벤트로 프로필을 바꿀 수 있다. [AC-12](../../requirements.md#인수-조건)가 충족되어 [RISK-M2-04](../../plan.md#위험)가 해소됐다. 난이도 프리셋은 가중치 세트를 바꾸는 것만으로 만들 수 있으며 실제 프리셋 값은 [TASK-33](../../plan.md#task-33-m2-검증-스테이지와-원작-대조-튜닝)의 대조 대상이다.

### 2026-08-20 — TASK-32 전투 UI 확장

- 수행 내용: [TASK-32](../../plan.md#task-32-전투-ui-확장)를 TDD 사이클로 구현했다. 선행 테스트(`tests/scenes/interaction.test.ts` 신규 14건)를 먼저 작성해 실패(Red)를 확인한 뒤 구현했다. 상태 기계가 책략·아이템·조사를 다루고, HUD가 책략치·경험치·상태이상과 하위 목록·연출 배너를 표시한다.
- 변경 파일: `src/scenes/BattleInteraction.ts`(메뉴·하위 목록·대상 지정 확장), `src/ui/BattleHud.ts`(표시 항목·하위 목록·배너), `src/scenes/BattleScene.ts`(배선·이벤트 요약), `index.html`(스타일), `tests/scenes/interaction.test.ts`
- 발견 사항:
  - **그리기 전에 판정을 다 옮기면 화면 코드가 거의 남지 않는다.** 메뉴 가용 여부·대상 가능 칸·아이템 사용 조건이 전부 코어의 `tacticRejection`·`itemRejection`을 그대로 부르므로, 씬은 "무엇을 그릴지"만 정한다. [RISK-M0-01](../../plan.md#위험)(렌더는 자동 검증이 어렵다)의 완화가 실제로 작동한 셈이다 — 새로 더한 14건이 전부 화면 없이 돈다.
- 결정과 이유:
  - **메뉴 항목을 상태 기계가 만든다.** 씬이 "공격 가능한가"를 다시 판정하면 코어와 갈라진다. `ui.menu()`가 항목과 **쓸 수 없는 이유**까지 돌려주고, 화면은 그것을 흐리게 그리고 이유를 툴팁에 붙인다 — 항목을 감추지 않는 것은 M1이 세운 원칙 그대로다.
  - **책략·아이템은 하위 목록을 한 번 더 거친다.** 무엇을 쓸지 고르지 않고 대상 지정으로 넘어가면 되돌릴 방법이 없다. 하위 목록에서 취소하면 목록만 닫히고 부대 선택은 유지된다.
  - **대상 지정 가능 칸을 맵 전수로 계산한다.** 책략은 사거리·지형·날씨가 함께 걸려 "칠 수 있는 칸"이 사각형이 아니다. 맵을 훑으며 같은 판정 함수를 부르는 편이 규칙을 두 번 적는 것보다 안전하고, 맵 크기가 수백 칸이라 비용도 문제가 되지 않는다.
  - **연출은 한 줄 배너로 최소 구현했다.** 계획은 `DialogueBox`·`DuelCutin`을 별도 컴포넌트로 적었지만, M2의 완료 판정은 "화면에 한 번 이상 표시된다"([VER-09](../../plan.md#검증-계획))이고 컷인 연출의 구체적 형태는 [상세 스펙 §8.3]이 M5(그래픽)와 함께 다룬다. 지금 필요한 것은 이벤트가 실제로 발동했음을 화면에서 확인할 수 있는 최소 표시이므로 대화·일기토를 배너 한 줄로 옮겼다. 컴포넌트 분리는 표시할 내용이 늘어날 때 한다(YAGNI).
- 실행한 검증:

  | 검증 | 명령 | 결과 | 증거 |
  |---|---|---|---|
  | 선행 테스트 실패 확인 (Red) | `npx vitest run tests/scenes/interaction.test.ts` | 의도한 실패 | `menu`·`tactics`·`chooseTactic` 미존재 |
  | 메뉴 가용 판정 | `npm test` | 성공 | 사거리에 적이 없으면 공격 비활성 / 책략치가 모자라면 각 책략에 이유 / 소지품이 없거나 장비뿐이면 아이템 비활성 / 보물 칸에서만 조사 활성 |
  | 대상 지정 상태 기계 | `npm test` | 성공 | 책략 선택 → `targeting` / 취소 시 행동 메뉴 복귀 / 고를 수 없는 칸은 무시 / 확정 시 적용 후 선택 해제 / 대상이 필요 없는 아이템은 즉시 적용 |
  | 전체 테스트 | `npm test` | 성공 | 18파일 470건 통과 (TASK-31 시점 456건 + 신규 14건) |
  | 타입 검사·데이터 검증·빌드 | `npx tsc --noEmit` / `npm run validate` / `npm run build` | 성공 | 출력 없음 / `데이터 검증 통과` / `✓ built in 1.00s` |
  | 화면 확인 | — | **미수행** | 그리기·연출은 [TASK-33](../../plan.md#task-33-m2-검증-스테이지와-원작-대조-튜닝)의 VER-09에서 검증 스테이지와 함께 확인한다 |

- 결과: 조작·표시가 코어 규칙과 한 판정을 공유한 채 화면에 붙었다. 남은 것은 [TASK-33](../../plan.md#task-33-m2-검증-스테이지와-원작-대조-튜닝)의 검증 스테이지 완주(VER-09)와 상수 대조(VER-14)다.

### 2026-08-20 — TASK-33 M2 검증 스테이지와 원작 대조 튜닝

- 수행 내용: M2가 더한 규칙을 모두 발동시키는 검증 스테이지 `data/scenario/stages/stage-m2.json`을 만들고, 그 스테이지를 조작 계층으로 완주하는 자동 검증을 남겼다. **VER-14는 성공**, **VER-09는 실행 환경 부재로 차단**되었다. 원작 실측 자료가 없어 `[검증]` 상수는 하나도 확정하지 못했고 전부 미확정으로 남겼다.
- 변경 파일: `data/scenario/stages/stage-m2.json`(신규), `tests/scenes/m2-stage.test.ts`(신규), `README.md`
- 발견 사항:
  - **VER-09를 이 PC에서 실행할 수 없다.** [M1이 쓴 방식](../20260817-m1-battle-prototype/work-log.md#재개-지점)은 `npm run dev` + 헤드리스 Chrome에 CDP로 입력을 주입하는 것인데, 이 맥미니에는 Chrome·Chromium·Edge·Brave가 모두 설치되어 있지 않다(`/Applications`와 `PATH` 확인). 브라우저를 새로 설치하는 것은 실행 환경을 바꾸는 일이라 사용자 승인 없이 하지 않았다(wf-implement §2.3).
  - **원작 실측 자료가 이 PC에 없다.** `reference/`는 `.gitignore` 대상이라([NFR-07](../../requirements.md#비기능-요구사항)) 이관된 저장소에 따라오지 않았다. [RISK-M2-01](../../plan.md#위험)이 예상한 상황이며, 계획대로 **값을 지어내지 않고** 미확정 목록에 그대로 남겼다.
  - **AI만으로는 조사·승급·아이템 사용이 일어나지 않는다.** 적 AI는 그 행동들을 계획하지 않기 때문이다(플레이어가 고르는 행동이다). 그래서 자동 완주 테스트는 앞부분을 화면과 같은 조작 계층(`BattleInteraction`)으로 직접 몰고, 나머지를 AI에 맡긴다.
  - **시드 데이터에 반격 병과가 없어 완주 중 반격이 일어나지 않는다.** 어느 원작 병과가 `counterAttack`을 갖는지가 `[검증]` 항목이라 지정하지 않았기 때문이다([TASK-27 기록](#2026-08-18--task-27-병과-플래그와-반격)). 반격 규칙 자체는 단위 테스트가 덮는다.
- 결정과 이유:
  - **검증 스테이지를 규칙이 발동하도록 설계했다.** 숲(화계 보정)·산과 황무지(지계)·다리(수계)·마을과 성문(거점 회복·공격 책략 무효)·강(통행 불가)을 배치하고, 동적 날씨·보물 2곳·이벤트 4종(대화·일기토·증원과 프로필 교체·도달)·AI 프로필 3종을 넣었다. 장비 대신 규칙 발동이 목적이므로 [RISK-M0-02](../../plan.md#위험)대로 이것은 파이프라인 검증용이며 원작 데이터가 아니다.
  - **장비의 사기를 26으로 시작시켰다.** 혼란 판정(§1.4)은 사기가 임계값 30 미만일 때만 도는데, 만점에서 시작하면 한 판 안에 그 규칙이 발동하지 않는다. 검증 스테이지의 목적이 "규칙을 모두 발동시키는 것"이므로 데이터로 그 상황을 만들었다.
  - **자동 완주가 VER-09를 대신하지 않는다.** 새 테스트는 화면이 부르는 것과 **같은 조작 계층**을 쓰지만 그리기와 연출은 지나가지 않는다. 그래서 파일 주석과 아래 표에 둘을 명확히 갈라 적었다 — 자동 완주는 "규칙이 발동한다"까지, 화면 확인은 "화면에 보인다"까지다.
- 실행한 검증:

  | 검증 | 명령 | 결과 | 증거 |
  |---|---|---|---|
  | 검증 스테이지 로드 | `npm run validate` | 성공 | `병과 7, 책략 8, 아이템 8, 무장 6, 지형 9, 스테이지 2` |
  | 규칙 발동 자동 완주 | `npx vitest run tests/scenes/m2-stage.test.ts` | 성공 | 한 판 안에서 15종이 발동 — 보물 조사·승급·아이템 사용·이벤트·대화·공격·사기 변동·경험치·레벨업·혼란·책략·일기토 시작/결과·회복·날씨 변화. 승패도 났다(무한 루프 없음) |
  | 여러 시드에서 재현 | 시드 1·7·42·20260820으로 AI 완주 | 성공 | 전부 4~6턴에 승리로 끝나고 책략·일기토·레벨업·날씨가 발동. 시드마다 발동 조합이 달라 난수가 실제로 쓰인다 |
  | **VER-14 ①**(NFR-06, 정적) | `src/core/battle/*.ts`·`eventRunner.ts`의 숫자 리터럴 확인 | 성공 | 튜닝 값으로 볼 만한 리터럴 없음. 남은 것은 정의역(사기 하한 0·확률 정규화 `range(0, 100)`·배수식의 1·평지 코스트 1)뿐이다 |
  | **VER-14 ②**(NFR-06, 동적) | `combat-config.json`의 `baseDamage`를 3배로, `confusion.chance`를 0으로 바꾸고 재실행 | 성공 | **코드를 한 줄도 고치지 않고** 전투가 4~5턴 → 2턴으로 줄고 혼란이 사라졌다. 원본 복원 후 원래 결과로 돌아왔다 |
  | **VER-09**(AC-03, 화면 완주) | — | **차단(미수행)** | 이 PC에 Chromium 계열 브라우저가 없다. 브라우저를 갖춘 환경에서 `npm run dev` 후 M1과 같은 CDP 하네스로 실행해야 한다 |
  | 전체 테스트·타입 검사·빌드 | `npm test` / `npx tsc --noEmit` / `npm run build` | 성공 | 19파일 472건 / 출력 없음 / `✓ built in 1.05s` |

- 확정한 상수: **없음**. 원작 실측 자료를 확보하지 못했다.
- 미확정으로 남긴 상수와 근거(전부 [상세 스펙](../../../yeonggeoljeon-remake-spec-detail.md)의 `[검증]` 표기이며 값은 초기 추정치다):

  | 상수 | 초기값 | 정본이 남긴 상태 |
  |---|---|---|
  | `confusion.chance` | 0.5 | §1.4가 "확률 판정"이라고만 적었다 |
  | `exp.divisor` | 10 | §1.6의 `expDivisor`가 `[검증]` |
  | `exp.defeatBonus` | 45 | §1.6이 준 "40~50" 범위 안의 값 |
  | `duel.winnerMorale` | 10 | §7.2가 "아군 전체 사기 +N" |
  | `ai.damage`~`ai.approach`(W1~W6) | 1 / 500 / 0.5 / 80 / 200 / 30 | §5.2가 가중치 값을 주지 않았다 |
  | `tactic.strongholdBlocks` | `["damage"]` | §1.5가 사기저하·혼란 포함 여부를 `[검증]`으로 남겼다 |
  | `counterAttack` 보유 병과 | 없음 | §1.1이 "대상 병과 확인 필요" |
  | `noTerrainPenalty`의 의미 | 이동 코스트만 | 스펙·설계 어디에도 정의가 없다 |

- 결과: 검증 스테이지와 규칙 발동 증거가 남았고 [VER-14](../../plan.md#검증-계획)가 충족됐다. [AC-03](../../requirements.md#인수-조건)의 "규칙 누락 없이 동작한다"는 자동 완주까지 확인됐고, "화면에서 완주한다"는 브라우저가 있는 환경의 몫으로 남는다. 상수 튜닝은 자료가 들어오는 시점에 **데이터만 고치면 되는 상태**(VER-14 ②로 확인)이므로 코드 작업이 다시 필요하지 않다.

### 2026-08-21 — TASK-34 자체 리뷰·전체 검증·통합

- 수행 내용: M2 전체 변경을 [wf-implement §3.5](../../plan.md#task-34-자체-리뷰전체-검증통합)의 네 축으로 리뷰하고, 발견한 결함 2건을 고친 뒤 자동 게이트 4종을 다시 실행했다.
- 변경 파일: `src/core/battle/ai.ts`(쓰이지 않는 export 제거), `src/main.ts`(여는 전장을 명시), `docs/plan.md`, 본 기록
- 리뷰에서 발견하고 고친 것:
  - **어느 전장이 열리는지가 파일 이름 순서에 달려 있었다.** `main.ts`가 `data.stages[0]`을 쓰는데 [TASK-33](#2026-08-20--task-33-m2-검증-스테이지와-원작-대조-튜닝)이 `stage-m2.json`을 더하면서, 사전순으로 앞서는 M2 검증 전장이 `npm run dev`의 기본이 **조용히** 되었다. 데이터를 더한 것만으로 화면 동작이 바뀐 것이라 `DEFAULT_STAGE` 상수로 무엇을 여는지 명시하고, 없으면 첫 스테이지로 물러나게 했다. M3에서 캠페인이 이 선택을 대신한다.
  - **쓰이지 않는 export가 하나 남아 있었다.** [TASK-31](#2026-08-19--task-31-2단계-적-ai)에서 "화면이 쓸 것"이라며 `ai.ts`에 `reachableTargets`를 넣었지만 아무도 부르지 않는다. 최소 구현 원칙(요청되지 않은 코드를 두지 않는다)대로 지웠다.
- 리뷰에서 확인만 하고 넘어간 것:
  - **요구사항·설계**: M2 범위의 FR-04~09·FR-11·FR-13·FR-18과 DES-01·03·05·06·12·13이 모두 구현으로 이어졌다. 승인된 기준선을 벗어난 변경은 없고, 계획과 달라진 판단은 각 작업 기록의 "결정과 이유"에 남아 있다.
  - **정확성·안정성**: `applyCommand`의 거부 지점 14곳이 전부 상태 변경보다 앞선다(정적 확인) — 거부된 커맨드는 상태를 바꾸지 않는다. `runEvents`는 `applyOne`이 던지면 아예 돌지 않아 부분 적용 상태가 남지 않는다. `runAiPhase`는 행동할 부대 목록을 먼저 고정하고 매번 다시 조회하므로, 이벤트가 부대를 지워도 유한하게 끝난다.
  - **보안·운영**: 외부 입력·인증·비밀 정보가 없는 오프라인 게임이라 해당 항목이 없다. 데이터는 전부 zod 스키마와 참조 무결성 검사를 거친다(NFR-03). `src/`에 디버그 코드·임시 파일이 없고, 남은 `console.warn`/`console.error`는 M1이 세운 데이터 문제·부팅 실패 보고다.
  - **품질**: `src/core/`가 PixiJS·`node:`·DOM을 import하지 않는다(NFR-02, M0의 VER-03 재확인). 규칙 판정이 코어 한 곳에 있고 화면·AI가 같은 함수를 부른다 — `tacticRejection`·`itemRejection`을 조작 계층과 AI가 공유하는 것이 그 결과다. TDD를 적용하지 못한 변경은 없다(그리기·연출만 화면 확인으로 남겼고 그 이유를 [TASK-32 기록](#2026-08-20--task-32-전투-ui-확장)에 적었다).
- 실행한 검증:

  | 검증 | 명령 | 결과 | 증거 |
  |---|---|---|---|
  | 타입 검사 | `npx tsc --noEmit` | 성공 | 출력 없음 |
  | 전체 테스트 | `npm test` | 성공 | **19파일 472건** — M1 기준선 184건이 한 건도 깨지지 않았다 |
  | 데이터 검증 | `npm run validate` | 성공 | `병과 7, 책략 8, 아이템 8, 무장 6, 지형 9, 스테이지 2` |
  | 빌드 | `npm run build` | 성공 | `✓ built in 1.03s` |
  | 로직·렌더 분리 (NFR-02) | `src/core/`의 import 정적 확인 | 성공 | PixiJS·`node:`·`document`·`window` 참조 0건 |

- 결과: M2 구현이 로컬 작업 사본에 완결된 형태로 반영됐다. 남은 것은 실행 환경이 필요한 [VER-09](../../plan.md#검증-계획)와 원작 자료가 필요한 상수 대조이며, 둘 다 코드 작업이 아니다.

## 검증 범위와 환경

- 대상 기준선 또는 구현: [REQ-ygj-remake](../../requirements.md) v1 / [DESIGN-ygj-remake](../../design.md) v1의 M2 범위. TASK-21~32 완료, TASK-33 부분 완료, TASK-34 미착수.
...
- **성공**: 작업 14건(TASK-21~34)을 전부 수행했고 자동 게이트 4종이 모두 통과한다 — `npm test` **19파일 472건**, `npx tsc --noEmit`, `npm run validate`, `npm run build`. 검증 7건 중 **6건(VER-10·11·12·13·14·15)이 성공**했다. M1 기준선 184건이 한 건도 깨지지 않았다(공격 이벤트 배열을 정확히 일치로 단언하던 4건은 규칙 확장에 맞춰 의도를 유지한 채 포함 단언으로 고쳤다).
- **실패**: 없음.
- **차단**: **VER-09**(AC-03의 화면 완주) 1건. 실행 환경에 Chromium 계열 브라우저가 없어 수행하지 못했다 — 규칙이 빠졌거나 실패한 것이 아니다.

## 인수 조건별 결과

| 검증 ID | 인수 조건 | 방법·명령 | 결과 | 증거 |
|---|---|---|---|---|
| VER-09 | [AC-03](../../requirements.md#인수-조건) | M2 검증 스테이지 화면 완주(`npm run dev` + CDP 하네스) | **차단** | 이 PC에 Chromium 계열 브라우저가 없다. 규칙 발동 자체는 같은 조작 계층을 쓰는 자동 완주로 확인했다([TASK-33](#2026-08-20--task-33-m2-검증-스테이지와-원작-대조-튜닝)) |
| VER-10 | [AC-04](../../requirements.md#인수-조건) | 상성×지형×아이템 보정 단위 테스트 | 성공 | 무기·병법서 배수가 상성(기병→보병)·지형(숲 +10%)과 함께 걸리고, 장비가 없으면 M1 값 그대로다([TASK-25](#2026-08-18--task-25-아이템-데이터-계약과-장비-효과)) |
| VER-11 | [AC-12](../../requirements.md#인수-조건) | 적 페이즈 AI 100ms 측정 | 성공 | 적 10부대·20×20 맵과 책략 후보가 많은 배치 모두 100ms 미만([TASK-31](#2026-08-19--task-31-2단계-적-ai)) |
| VER-12 | [NFR-01](../../requirements.md#비기능-요구사항) | 고정 시드 재현 테스트 | 성공 | 혼란 판정·책략 명중·동적 날씨·일기토 `judge`·AI 선택이 각각 같은 시드에서 같은 결과를 낸다. 일기토는 난수 소비 횟수까지 고정했다(고정 승패 0회, `judge` 1회) |
| VER-13 | [AC-01](../../requirements.md#인수-조건) | 손상 데이터로 `npm run validate` exit 1 | 성공 | 책략→지형, 소모품→책략, 전환서→병과, 보물→아이템·좌표, 알 수 없는 이벤트 트리거를 각각 깨뜨려 `exit 1`과 해당 메시지를 확인했다. `forbiddenFor` 경고는 `exit 0`으로 갈린다 |
| VER-14 | [NFR-06](../../requirements.md#비기능-요구사항) | 상수 외부화 정적 확인 + config 변경 반영 | 성공 | 코어에 튜닝 리터럴 없음. `baseDamage` 3배·혼란 확률 0으로 바꾸자 **코드 수정 없이** 전투가 4~5턴 → 2턴이 되고 혼란이 사라졌다([TASK-33](#2026-08-20--task-33-m2-검증-스테이지와-원작-대조-튜닝)) |
| VER-15 | [FR-09](../../requirements.md#기능-요구사항)·[DES-13](../../design.md#컴포넌트와-책임) | 이벤트 DSL 단위 테스트 | 성공 | 트리거 9종이 각자 조건에서만 발동 / `once` 재발동 방지 / 동시 발동 배열 순서 / 조건 플래그 양방향 / `camp` 스코프 제외 / 액션 12종 적용 / `endBattle`이 승패를 덮어쓴다([TASK-29](#2026-08-19--task-29-전투-이벤트-dsl-인터프리터)) |

## 실패와 미수행 분석

- **VER-09(차단)**: 이 PC(맥미니)에 Chromium 계열 브라우저가 설치되어 있지 않다. [M1이 쓴 방식](../20260817-m1-battle-prototype/work-log.md#재개-지점)은 `npm run dev` + 헤드리스 Chrome CDP인데 그 실행기가 없다. 브라우저 설치는 실행 환경 변경이라 사용자 승인 없이 하지 않았다(wf-implement §2.3). **규칙 자체는 같은 조작 계층을 쓰는 자동 완주로 확인**했으므로([TASK-33](#2026-08-20--task-33-m2-검증-스테이지와-원작-대조-튜닝)) 남은 것은 "그리기와 연출이 화면에 보이는가"다.
- **실패한 검증은 없다.** 자동 게이트 4종과 검증 6건이 모두 성공했다.
...
M2 사이클이 끝난 시점의 스냅숏이다(wf-implement §7). 이 뒤로 [계획 문서의 트리](../../plan.md#계획-트리)가 축약되더라도 이 절의 내용은 바뀌지 않는다.

```text
[~] [작업] 20260818-m2-battle-complete — M2 전투 완성 ..... partial (14/14 구현, VER-09 차단)  depends: M1
    ├─ [✓] TASK-21 구현: 사기·혼란과 턴 시작 처리          completed  2026-08-18
    ├─ [✓] TASK-22 구현: 경험치·레벨업                    completed  2026-08-18
    ├─ [✓] TASK-23 구현: 책략 데이터 계약과 책략치(MP)     completed  2026-08-18
    ├─ [✓] TASK-24 구현: 책략 실행 커맨드                  completed  2026-08-18
    ├─ [✓] TASK-25 구현: 아이템 데이터 계약과 장비 효과    completed  2026-08-18
    ├─ [✓] TASK-26 구현: 아이템 사용과 승급·계열 전환      completed  2026-08-18
    ├─ [✓] TASK-27 구현: 병과 플래그와 반격                completed  2026-08-18
    ├─ [✓] TASK-28 구현: 보물·군량고 조사와 날씨           completed  2026-08-18
    ├─ [✓] TASK-29 구현: 전투 이벤트 DSL 인터프리터        completed  2026-08-19
    ├─ [✓] TASK-30 구현: 일기토                            completed  2026-08-19
    ├─ [✓] TASK-31 구현: 2단계 적 AI                       completed  2026-08-19
    ├─ [✓] TASK-32 구현: 전투 UI 확장                      completed  2026-08-20
    ├─ [~] TASK-33 검증: M2 검증 스테이지와 원작 대조 튜닝  partial (VER-14 성공, VER-09 차단)
    └─ [✓] TASK-34 통합: 자체 리뷰·전체 검증·통합          completed  2026-08-21
```

```mermaid
flowchart TD
    M2["M2 전투 완성 (14/14 구현)<br/>VER-09 차단"]:::gate
    M2 --> T21["TASK-21 사기·혼란·턴 시작 처리"]:::done
    M2 --> T22["TASK-22 경험치·레벨업"]:::done
    M2 --> T23["TASK-23 책략 데이터·책략치"]:::done
    M2 --> T24["TASK-24 책략 실행"]:::done
    M2 --> T25["TASK-25 아이템 데이터·장비 효과"]:::done
    M2 --> T26["TASK-26 아이템 사용·승급"]:::done
    M2 --> T27["TASK-27 병과 플래그·반격"]:::done
    M2 --> T28["TASK-28 보물 조사·날씨"]:::done
    M2 --> T29["TASK-29 전투 이벤트 DSL"]:::done
    M2 --> T30["TASK-30 일기토"]:::done
    M2 --> T31["TASK-31 2단계 적 AI"]:::done
    M2 --> T32["TASK-32 전투 UI 확장"]:::done
    M2 --> T33["TASK-33 검증 스테이지·원작 대조"]:::gate
    M2 --> T34["TASK-34 자체 리뷰·검증·통합"]:::done
...
- **완료 상태: 부분 완료.** 계획한 작업 14건을 전부 수행했고 검증 7건 중 6건이 성공했다. [VER-09](../../plan.md#검증-계획)만 실행 환경 부재로 차단되어 "완료"가 아니라 "부분 완료"로 보고한다(wf-implement §5).
- **완료한 내용**: 사기·혼란과 턴 시작 처리 파이프라인, 경험치·레벨업, 책략(데이터 계약·실행·지형/날씨 게이트), 아이템(데이터 계약·장비 효과·사용·승급·계열 전환), 병과 플래그와 반격, 보물 조사와 동적 날씨, 전투 이벤트 DSL(트리거 9종·액션 12종), 일기토, 2단계 적 AI(프로필 5종·가중치 스코어링), 전투 UI 확장, M2 검증 스테이지.
- **주요 변경**: `src/core/battle/`에 규칙 모듈 6개가 새로 생겼고(`morale`·`experience`·`tactics`·`items`·`duel` + 기존 확장), `src/core/campaign/eventRunner.ts`가 이벤트 DSL을 해석한다. 데이터는 `tactics.json`·`items.json`이 늘고 `stage` 계약이 보물·이벤트·동적 날씨로 넓어졌다. 전투 상수 30여 종이 전부 `combat-config.json`에 있다.
- **설계와 달라진 점**: 승인된 요구사항·설계를 벗어난 변경은 없다. 계획의 세부와 달라진 판단 4건은 [설계와 달라진 점](#설계와-달라진-점)에 모았다.
- **통합 상태**: 코드·테스트·데이터·문서가 함께 갱신되어 로컬 작업 사본에 완결됐고, 작업 단위마다 `origin/main`에 push했다.
- **남은 위험과 후속 작업**: 아래 [남은 위험](#남은-위험)·[미완료 항목](#미완료-항목) 참조.

## 남은 위험

- **[RISK-M2-01](../../plan.md#위험) 원작 실측값 미확보 — 현실화됨.** 자료(`reference/`)가 이 PC에 없어 `[검증]` 상수 8건을 확정하지 못했다. 계획대로 값을 지어내지 않고 목록으로 남겼으며, [VER-14](#인수-조건별-결과)로 "데이터만 고치면 되는 상태"임을 확인했으므로 자료가 들어오면 코드 작업이 필요 없다.
- **[RISK-M2-02](../../plan.md#위험) 규칙 간 회귀 — 해소.** 작업마다 M1 테스트 전량을 다시 돌렸고 마지막에도 472건이 통과한다. 규칙을 순수 함수로 나누고 상태 변경을 `applyCommand`와 턴 시작 파이프라인 두 경로로 모은 완화가 실제로 작동했다.
- **[RISK-M2-03](../../plan.md#위험) 연출 의존 검증 — 절반 해소.** 코어(트리거·액션·판정)는 단위 테스트로 고정했고([VER-15](#인수-조건별-결과)), 연출 재생은 VER-09가 차단되어 남아 있다.
- **[RISK-M2-04](../../plan.md#위험) AI 성능 — 해소.** 전탐색으로 바꾼 뒤에도 100ms 예산 안에 들었다([VER-11](#인수-조건별-결과)).
- **[RISK-M0-01](../../plan.md#위험) 렌더 자동 검증의 어려움 — 유효.** 판정을 코어로 옮겨 화면 없이 도는 테스트를 늘렸지만 그리기 자체는 여전히 눈으로 봐야 한다.
- **[RISK-M0-02](../../plan.md#위험) 시드 데이터를 원작 데이터로 오인 — 유효.** `tactics.json`·`items.json`·`stage-m2.json`은 전부 파이프라인 검증용 샘플이며 원작 입력은 M4다.
...
**승인된 요구사항·설계를 벗어난 변경은 없다.** 아래는 계획의 세부(변경 대상·검증 방법)와 달라진 판단이며 전부 승인 범위 안의 내부 구현 세부사항이다(wf-implement §4.1).

| 계획이 적은 것 | 실제 | 이유 |
|---|---|---|
| `morale.ts`는 변경 대상에 없음 | 새 파일을 만들었다 | 사기·혼란 규칙을 `damage.ts`·`movement.ts`와 같은 "규칙 영역 하나 = 파일 하나" 구조로 두었다([TASK-21](#2026-08-18--task-21-사기혼란과-턴-시작-처리)) |
| TASK-26에서 `hpMax`·`mpMax`가 새 병과 기준으로 갱신된다 | 갱신할 것이 없다 | 둘 다 무장·레벨에서 나오고 병과에 의존하지 않는다. 대신 승급 직후 새 사거리 규칙이 적용되는지를 고정했다([TASK-26](#2026-08-18--task-26-아이템-사용과-승급계열-전환)) |
| TASK-27에서 `damage.ts`의 지형 보정 무시 범위를 확정 | 손댈 자리가 없었다 | 현재 공식에서 지형은 방어측 보너스일 뿐 페널티가 아니다. 없는 규칙을 지어내지 않고 이동에만 적용했다([TASK-27](#2026-08-18--task-27-병과-플래그와-반격)) |
| TASK-32에서 `DialogueBox`·`DuelCutin`을 별도 컴포넌트로 | 배너 한 줄로 최소 구현 | 컷인의 구체적 형태는 [상세 스펙 §8.3]이 M5(그래픽)와 함께 다룬다. 지금 필요한 것은 이벤트 발동을 화면에서 확인할 수 있는 최소 표시다([TASK-32](#2026-08-20--task-32-전투-ui-확장)) |

**사용자 결정으로 확정한 규칙 1건**: 적 부대의 성장은 전투 중에는 쌓이되 다음 스테이지로 이관되지 않는다([수행 기록](#2026-08-18--task-22-후속-적-부대-경험치-규칙-확정)).

## 미완료 항목

- **[VER-09](#인수-조건별-결과)(AC-03의 화면 완주)** — 실행 환경에 Chromium 계열 브라우저가 없어 차단. 브라우저가 있는 PC에서 `npm run dev` 후 [M1과 같은 CDP 하네스](../20260817-m1-battle-prototype/work-log.md#재개-지점)로 실행하면 된다. 검증 전장은 `stage-m2`이며 `npm run dev`가 기본으로 연다.
- **미확정 상수 8건** — 전부 [상세 스펙](../../../yeonggeoljeon-remake-spec-detail.md)의 `[검증]` 항목이라 값을 지어내지 않았다([RISK-M2-01](#남은-위험)). 목록과 근거는 [TASK-33 수행 기록](#2026-08-20--task-33-m2-검증-스테이지와-원작-대조-튜닝)의 표에 있다.
- **`counterAttack`을 가진 시드 병과** — 어느 원작 병과가 반격하는지가 `[검증]`이라 지정하지 않았다. 규칙과 단위 테스트는 있으므로 데이터에 플래그를 넣기만 하면 동작한다.
- **M2 범위 밖으로 남긴 것**(계획대로): `camp` 스코프 이벤트·분기·세이브, `choice`·`gameOver`·`giveGold`·`joinOfficer` 액션, 상점·자금, 제3세력 페이즈, `officerLost` 재정의, 경험치 공유 토글의 UI 노출, 원작 콘텐츠 데이터 입력 — 각각 M3·M4가 맡는다.

## 재개 지점

- 다음 작업: **VER-09 실행**(브라우저가 있는 환경) → 그 뒤 M3(캠페인·세이브) 착수 판단
- 먼저 확인할 사항:
  1. `npm ci` 후 자동 게이트 4종(`npm test`·`npx tsc --noEmit`·`npm run validate`·`npm run build`)이 통과하는지 본다. `node_modules/`는 커밋되지 않는다.
  2. **VER-09**: `npm run dev`로 열리는 전장이 `stage-m2`(M2 검증 전장)다. 헤드리스 Chrome을 `--remote-debugging-port`로 띄우고 Node 내장 `WebSocket`으로 CDP에 붙어 캔버스 클릭·HUD 버튼·키 입력만 주입한다(앱 내부 상태에 손대지 않는다). 확인할 것은 책략·아이템·조사 메뉴, 사기·책략치·경험치·혼란 표시, 대화·일기토 배너, 승패 표시다.
  3. **M2가 세운 계약**(M3이 이어받는다): 규칙 판정은 전부 코어에 있고 화면·AI가 같은 함수를 부른다(`tacticRejection`·`itemRejection`). 사기는 `changeMorale`, 경험치는 `gainExp`, 이벤트는 `runEvents`를 지난다. `BattleState`는 세이브 가능한 값만 담으며 M2가 더한 필드는 `confused`·`exp`·`mp`·`mpMax`·`items`·`ai`·`aiParams`·`treasuresTaken`·`firedEvents`·`flags`·`forcedOutcome`·`weatherTurnsLeft`다.
  4. **M3이 채울 자리**: `beginPhase`의 ① 자동 저장, `eventRunner`의 `camp` 스코프와 `choice`·`gameOver`, `BattleState.flags`를 대신할 캠페인 상태, `main.ts`의 `DEFAULT_STAGE`를 대신할 스테이지 선택.
  5. 새 전투 상수는 `CombatConfigSchema`에 넣고 `data/config/combat-config.json`·`tests/data/fixtures.ts`의 `validCombatConfig` 두 곳을 함께 갱신한다. 데이터 파일을 더하면 로더·validate CLI·브라우저 3경로(`loader.ts`·`scripts/readGameData.ts`·`browserData.ts`)와 `scripts/validate.ts` 요약 줄을 모두 고친다.
  6. M1이 확정한 계약은 [M1 재개 지점](../20260817-m1-battle-prototype/work-log.md#재개-지점)에 그대로 유효하다.
- 필요한 명령 또는 파일: `npm ci` / `npm test` / `npm run validate` / `npm run dev` / `npm run build`. **현재 개발 PC는 맥미니(macOS), Node.js v26.7.0**이며 Chromium 계열 브라우저는 설치되어 있지 않다.

## 인계

- 다음 단계 또는 워크플로우: VER-09 실행 후 M3 착수 판단. M3는 새 제품 결정(캠페인 분기·세이브 포맷)을 포함하므로 [wf-design](../../../.claude/skills/wf-design/SKILL.md)으로 기준선을 먼저 확인한다.
- 시작 조건: 충족 — 기준선 v1 승인, M0·M1 완료, M2 구현 14/14 완료, 자동 게이트 4종 통과.
...
- 완료된 항목: M2 계획 수립과 **TASK-21~34 전부**. 검증 VER-10·11·12·13·14·15 성공.
- 미완료 항목: **VER-09**(브라우저 부재로 차단), 미확정 상수 8건의 원작 대조.
- 차단 요인: **VER-09** — 실행 환경에 Chromium 계열 브라우저가 없다. 브라우저를 설치하거나 브라우저가 있는 PC에서 실행하면 해소된다.
- 다음 행동: 브라우저가 있는 환경에서 VER-09를 실행하고 결과를 이 기록의 [인수 조건별 결과](#인수-조건별-결과)에 반영한다.
- 미커밋 변경: 없음 — TASK-21~34의 모든 변경을 작업 단위로 `origin/main`에 push했다(`95c3569`·`0d19b76`·`9e0d79b`·`253cf1b`·`e4d2091`·`95a1c60`·`bc1fc69`·`58ed355`·`1063292`·`adac50b`·`df06c2a`·`1b3625b`·`61cc13d`와 TASK-34 커밋).
- 재개 프롬프트: 작업 20260818-m2-battle-complete의 M2 구현이 전부 끝났고 VER-09만 차단되어 있다. docs/work/20260818-m2-battle-complete/work-log.md의 재개 지점을 읽고 VER-09를 실행하거나 M3 착수를 판단하라.
