...
[~] [작업] 20260818-m2-battle-complete — M2 전투 완성 ..... in-progress (5/14)  depends: M1
...
flowchart TD
    ...
    M2["M2 전투 완성 (5/14)<br/>20260818-m2-battle-complete"]:::active
    M0 -. depends .-> M1
    M1 -. depends .-> M2
...
- 완료된 항목: M0 사이클 TASK-01~09(검증 VER-01~04), M1 사이클 TASK-10~20(검증 VER-05~08), **M2 계획 수립**(TASK-21~34, VER-09~15, 계획 트리 재생성 — 2026-08-18), **M2 [TASK-21](#task-21-사기혼란과-턴-시작-처리)**(사기·혼란·턴 시작 처리)·**[TASK-22](#task-22-경험치레벨업)**(경험치·레벨업)·**[TASK-23](#task-23-책략-데이터-계약과-책략치mp)**(책략 데이터 계약과 책략치)·**[TASK-24](#task-24-책략-실행-커맨드)**(책략 실행 커맨드)·**[TASK-25](#task-25-아이템-데이터-계약과-장비-효과)**(아이템 데이터 계약과 장비 효과 — 전부 2026-08-18)
- 미완료 항목: M2 구현 TASK-26~34(5/14 완료), M3~M6 전부
- 차단 요인: 없음
- 다음 행동: [TASK-26 아이템 사용과 승급·계열 전환](#task-26-아이템-사용과-승급계열-전환)의 선행 테스트(`tests/battle/items.test.ts`)를 먼저 작성해 실패를 확인한 뒤 구현한다. 착수 시 TASK-26의 상태를 `in-progress`로 바꾸고 같은 변경에서 [계획 트리](#계획-트리)를 재생성한다
