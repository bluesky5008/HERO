...
- 현재 결론 또는 상태: **M0 사이클 완료**(TASK-01~09, 검증 4/4 성공, [기록](./work/20260817-m0-skeleton/work-log.md)). **M1 사이클 완료**(TASK-10~20, 검증 VER-05~08 4/4 충족, [기록](./work/20260817-m1-battle-prototype/work-log.md)) — 화면에서 전투 한 판을 승리까지 완주했고 테스트 184건·타입 검사·데이터 검증·빌드가 모두 성공했다. **M2 사이클 진행 중**(3/14) — 작업 14건(TASK-21~34)과 검증 7건(VER-09~15)을 아래에 정의했고, [TASK-21](#task-21-사기혼란과-턴-시작-처리)(사기·혼란·턴 시작 처리)·[TASK-22](#task-22-경험치레벨업)(경험치·레벨업)·[TASK-23](#task-23-책략-데이터-계약과-책략치mp)(책략 데이터 계약과 책략치)·[TASK-24](#task-24-책략-실행-커맨드)(책략 실행 커맨드)·[TASK-25](#task-25-아이템-데이터-계약과-장비-효과)(아이템 데이터 계약과 장비 효과)·[TASK-26](#task-26-아이템-사용과-승급계열-전환)(아이템 사용과 승급·계열 전환)을 완료했다(2026-08-18, 테스트 360건·validate·build 통과).
- 다음 행동: [TASK-27](#task-27-병과-플래그와-반격)부터 계획 순서대로 구현한다(wf-implement §3.3 TDD 사이클). 진행 상태는 이 문서의 작업 목록과 [M2 작업 기록](./work/20260818-m2-battle-complete/work-log.md)에 갱신한다.
...
[~] [작업] 20260818-m2-battle-complete — M2 전투 완성 ..... in-progress (6/14)  depends: M1
...
flowchart TD
    ...
    M2["M2 전투 완성 (6/14)<br/>20260818-m2-battle-complete"]:::active
    M0 -. depends .-> M1
    M1 -. depends .-> M2
...
- 완료된 항목: M0 사이클 TASK-01~09(검증 VER-01~04), M1 사이클 TASK-10~20(검증 VER-05~08), **M2 계획 수립**(TASK-21~34, VER-09~15, 계획 트리 재생성 — 2026-08-18), **M2 [TASK-21](#task-21-사기혼란과-턴-시작-처리)**(사기·혼란·턴 시작 처리)·**[TASK-22](#task-22-경험치레벨업)**(경험치·레벨업)·**[TASK-23](#task-23-책략-데이터-계약과-책략치mp)**(책략 데이터 계약과 책략치)·**[TASK-24](#task-24-책략-실행-커맨드)**(책략 실행 커맨드)·**[TASK-25](#task-25-아이템-데이터-계약과-장비-효과)**(아이템 데이터 계약과 장비 효과)·**[TASK-26](#task-26-아이템-사용과-승급계열-전환)**(아이템 사용과 승급·계열 전환 — 전부 2026-08-18)
- 미완료 항목: M2 구현 TASK-27~34(6/14 완료), M3~M6 전부
- 차단 요인: 없음
- 다음 행동: [TASK-27 병과 플래그와 반격](#task-27-병과-플래그와-반격)의 선행 테스트(`tests/battle/{commands,movement}.test.ts`)를 먼저 작성해 실패를 확인한 뒤 구현한다. 착수 시 TASK-27의 상태를 `in-progress`로 바꾸고 같은 변경에서 [계획 트리](#계획-트리)를 재생성한다
